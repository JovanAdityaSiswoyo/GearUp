...
new class extends Component
{
    ...
    public function login()
    {
        ...
        if ($admin && Auth::guard('admin')->attempt(['email' => $this->email, 'password' => $this->password], $this->remember)) {
            request()->session()->regenerate();
            return redirect()->route('admin.dashboard');
        }
        
        // Try to login as Officer
        $officer = Officer::where('email', $this->email)->first();
        if ($officer && Auth::guard('officer')->attempt(['email' => $this->email, 'password' => $this->password], $this->remember)) {
            request()->session()->regenerate();
            return redirect()->route('officer.dashboard');
        }
