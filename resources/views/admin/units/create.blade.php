...
<head>
    ...
    <title>Add New Unit - Admin</title>
...
<body class="bg-gray-50">
    <div class="min-h-screen">
        ...
        <div class="ml-64">
            @include('admin.partials.header', ['title' => 'Add New Unit'])
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select id="id_product" name="id_product" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('id_product') border-red-500 @enderror">
                                        ...
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}" {{ (old('id_product', request('product')) == $product->id) ? 'selected' : '' }}>
                                                {{ $product->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select id="status" name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('status') border-red-500 @enderror">
                                        ...
                                        <option value="lost_scrapped" {{ old('status') == 'lost_scrapped' ? 'selected' : '' }}>Lost/Scrapped</option>
                                    </select>
                                    @error('status')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="date" id="last_maintenance_at" name="last_maintenance_at" value="{{ old('last_maintenance_at') }}"
                                ...
                                <div>
                                    ...
                                    <textarea id="notes" name="notes" rows="4"
                                              ...
                                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                                    @error('notes')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
